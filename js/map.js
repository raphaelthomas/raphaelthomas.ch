var size = parseFloat($("#map").css("height"));
...
d3.json("/data/location.json", function(error, data) {
    ...
    $("#map-globe").one("click", function() {
        $("#typed-text").typed({
            strings: ["^2500Last time seen^1000 " + jQuery.timeago(new Date(data.time * 1000)) + (data.location ? "^500 somewhere in ^1000" + data.location : '')],
            typeSpeed: 50,
            startDelay: 1000,
            callback: function() {
                $("#location-description #typed").delay(5000).fadeOut(5000);
            },
        }).fadeIn(1000);
    });
});
